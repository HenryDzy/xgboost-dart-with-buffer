...
class HostDeviceVectorImpl {
 public:
  ...
  HostDeviceVectorImpl(const HostDeviceVectorImpl<T>& that) :
    device_{that.device_},
    data_h_{that.data_h_},
    gpu_access_{that.gpu_access_} {
      data_d_ = that.data_d_ ? std::make_unique<dh::DeviceUVector<T>>(*that.data_d_) : nullptr;
    }
...
HostDeviceVector<T>& HostDeviceVector<T>::operator=(const HostDeviceVector<T>& other) {
  ...
  std::unique_ptr<HostDeviceVectorImpl<T>> new_impl(
      new HostDeviceVectorImpl<T>(*other.impl_));
  delete impl_;
